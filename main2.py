...
for i in range(10):
    for line in open('raw'+ str(i) +'.json', 'r', encoding='utf8'):
        ...
        for item in (tweet_en_cours["entities"]["user_mentions"]):
            print(item)
            # if urls == "":
            #     urls = item["url"]
            # else:
            #     urls += ", " + item["url"]
        try :
            urls = tweet_en_cours["entities"]['url']
        ...
        try :
            ...
            ORM2.session.add(tweet)
            ORM2.session.commit()
            ORM2.session.flush()
            ORM2.session.refresh(tweet)
        except Exception as e:
            print(e)
            print("erreur")
...
#
# finalDf.to_csv('./finalDF.csv', encoding='utf-8')
print("ok")